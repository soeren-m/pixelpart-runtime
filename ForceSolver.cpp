	...
	for(uint32_t p = 0; p < numParticles; p++) {
		...
		switch(forceField.type) {
			case ForceField::Type::point: {
				floatd distance = glm::distance(forceFieldCenter, particles.globalPosition[p]);

				if(distance < forceFieldSize.x || forceFieldSize.x < 0.0) {
					gridCellX = (forceFieldSize.x > 0.0)
						? static_cast<int32_t>((particles.globalPosition[p].x - (forceFieldCenter.x - forceFieldSize.x)) / (forceFieldSize.x * 2.0) * static_cast<floatd>(forceField.gridSize[0]))
						: 0;
					gridCellY = (forceFieldSize.x > 0.0)
						? static_cast<int32_t>((particles.globalPosition[p].y - (forceFieldCenter.y - forceFieldSize.x)) / (forceFieldSize.x * 2.0) * static_cast<floatd>(forceField.gridSize[1]))
						: 0;
					gridCellZ = (forceFieldSize.x > 0.0)
						? static_cast<int32_t>((particles.globalPosition[p].z - (forceFieldCenter.z - forceFieldSize.x)) / (forceFieldSize.x * 2.0) * static_cast<floatd>(forceField.gridSize[2]))
						: 0;

					forceVector = (distance > 0.0001)
						? glm::normalize(forceFieldCenter - particles.globalPosition[p]) / std::max(distance, forceFieldSize.x * 0.01)
						: vec3d(0.0);
				}
			...
			case ForceField::Type::area: {
				vec3d rotatedParticlePosition = forceFieldCenter + vec3d(forceFieldOrientationMatrix * vec4d(particles.globalPosition[p] - forceFieldCenter, 1.0));
				...
				((rotatedParticlePosition.z > forceFieldCenter.z - forceFieldSize.z && rotatedParticlePosition.z < forceFieldCenter.z + forceFieldSize.z) || forceFieldSize.z < 0.0)) {
					...
					forceVector = vec3d(forceDirectionMatrix * worldUpVector4);
				}
		...
		forceVector *= particleType.weight.get(particles.life[p]);
